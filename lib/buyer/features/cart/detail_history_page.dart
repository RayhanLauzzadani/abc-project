...
class _DetailHistoryPageState extends State<DetailHistoryPage> {
      ...
      body: SafeArea(
        child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            final displayId = (invoiceRaw != null && invoiceRaw.isNotEmpty) ? invoiceRaw : realOrderId;
            ...
            final alreadyRated = (data['buyerRated'] ?? data['rated'] ?? false) as bool;
            final bool showReviewButton = orderStatus == OrderStatus.selesai && !alreadyRated;

            // stream info toko (logo dsb.)
            final storeStream = storeId.isEmpty
                ? const Stream<DocumentSnapshot<Map<String, dynamic>>>.empty()
                : FirebaseFirestore.instance.collection('stores').doc(storeId).snapshots();
            ...
            return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, storeSnap) {
                ...
                final storeAddress = (data['storeAddress'] ?? store['address'] ?? '-') as String? ?? '-';
                final storePhone = (data['storePhone'] ?? store['phone'] ?? '-') as String? ?? '-';
                                ...
                                ClipRRect(
                                  ...
                                  child: storeLogo.isNotEmpty
                                      ? Image.network(
                                          ...
                                          height: 54,
                                          fit: BoxFit.cover,
                                          errorBuilder: (_, __, ___) => _storePh(),
                                        )
                                      : _storePh(),
                                ),
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text('#$displayId',
                                          style: GoogleFonts.dmSans(
                                              fontSize: 13.3, color: const Color(0xFF888888))),
                                    ],
                                  ),
                                ),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              children: items.map((it) {
                                ...
                                final note = (it['variant'] ?? it['note'] ?? '') as String;
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        Text('Metode Pembayaran',
                                            style:
                                                GoogleFonts.dmSans(fontSize: 13, color: const Color(0xFF828282))),
                                        const Spacer(),
                                        Text(methodText,
                                            style: GoogleFonts.dmSans(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (serviceFee > 0) ...[
                                        ...
                                        _buildFeeRow('Biaya Layanan', serviceFee, bold: true),
                                      ],
                                      const SizedBox(height: 3),
  ...
  Map<String, dynamic> _mapOrderToTransaction({
    ...
    String? buyerName,    // penerima
  }) {
    final rawStatus =
        ((data['status'] ?? data['shippingAddress']?['status'] ?? 'PLACED') as String).toUpperCase();

    final uiStatus = (rawStatus == 'COMPLETED' ||
            rawStatus == 'DELIVERED' ||
            rawStatus == 'SUCCESS' ||
            rawStatus == 'SETTLED')
        ? 'Selesai'
    ...
    final serviceFee = ((amts['serviceFee'] ?? amts['service_fee'] ?? 0) as num).toInt(); // ➕
    final tax = ((amts['tax'] as num?) ?? 0).toInt();
    final total = ((amts['total'] as num?) ?? (subtotal + shipping + serviceFee + tax)).toInt();
    ...
    return {
      ...
      'amounts': {
        'subtotal': subtotal,
        'shipping': shipping,
        'serviceFee': serviceFee, // ➕ dikirim ke halaman nota
        'tax': tax,
        'total': total,
      },
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(text,
              style: GoogleFonts.dmSans(fontWeight: FontWeight.w600, color: color, fontSize: 13.2)),
        ],
      ),
    );
  }
